...
void MainWindow::connectToServer(const session::SessionConfig &config,
    ...
    // cannot run anything on the workstation unless the user explicitly
    // enabled it in session settings.
    session->commandRunner = new core::CommandRunner(session->container);
    ...
    session->commandHandler->setCommandRunner(session->commandRunner);
