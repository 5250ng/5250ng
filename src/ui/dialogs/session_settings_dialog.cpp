...
void SessionSettingsDialog::setupUI() {
    ...
    connect(okBtn, &QPushButton::clicked, this, &SessionSettingsDialog::onOk);
...
QWidget *SessionSettingsDialog::buildBrightnessSection() {
    ...
    auto addSlider = [&](const QString &label, QSlider *&slider, QLabel *&valLabel, int initial) {
        QHBoxLayout *row = new QHBoxLayout();
        row->addWidget(new QLabel(label));
        slider = new QSlider(Qt::Horizontal);
        slider->setRange(50, 150); // 0.5 to 1.5
        slider->setValue(initial);
        valLabel = new QLabel(QString::number(initial / 100.0, 'f', 2));
        row->addWidget(slider, 1);
        row->addWidget(valLabel);
        v->addLayout(row);
    };

    addSlider("Brightness:", m_brightnessSlider, m_brightnessLabel, 100);
    addSlider("Saturation:", m_saturationSlider, m_saturationLabel, 100);

    // Saturation can go up to 2.0
    ...
    connect(m_brightnessSlider, &QSlider::valueChanged, this, [this](int val) {
        m_brightnessLabel->setText(QString::number(val / 100.0, 'f', 2));
        onThemePropertyChanged();
    });
    connect(m_saturationSlider, &QSlider::valueChanged, this, [this](int val) {
        m_saturationLabel->setText(QString::number(val / 100.0, 'f', 2));
        onThemePropertyChanged();
    });
...
void SessionSettingsDialog::loadThemeToUI(const ui::themes::TerminalTheme &theme) {
    ...
    m_brightnessSlider->setValue(static_cast<int>(theme.globalBrightness * 100));
    m_saturationSlider->setValue(static_cast<int>(theme.globalSaturation * 100));
    m_brightnessLabel->setText(QString::number(theme.globalBrightness, 'f', 2));
    m_saturationLabel->setText(QString::number(theme.globalSaturation, 'f', 2));
