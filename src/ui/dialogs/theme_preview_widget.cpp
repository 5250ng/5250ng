...
void ThemePreviewWidget::paintEvent(QPaintEvent *event) {
    ...
    for (int row = 0; row < kPreviewRows; ++row) {
        for (int col = 0; col < kPreviewCols; ++col) {
            ...
            painter.setPen(fg);
            painter.drawText(cellRect, Qt::AlignCenter, cell.ch);
    ...
    {
        QColor sepColor = m_theme.columnSeparatorColor.isValid()
        ...
        for (int c = 10; c < kPreviewCols; c += 10) {
            ...
            painter.drawLine(x, offsetY, x, offsetY + totalH);
        }
    }

    // Draw selection highlight on "S1051PC3" (row 2, cols 23-30) to show selection colors
    {
        QColor selBg = m_theme.selectionBackground.isValid()
        ...
        for (int c = 23; c < 31 && c < kPreviewCols; ++c) {
            ...
            painter.fillRect(selRect, selBg);
        }
    }
    ...
    QRect cursorCellRect(offsetX + kCursorCol * cellW, offsetY + kCursorRow * cellH,
                         cellW, cellH);
    renderCursor(painter, cursorCellRect);

    // Apply CRT effects if enabled
    if (m_theme.crtEffectEnabled) {
        renderCRTEffect(painter, gridArea);
    }
...
void ThemePreviewWidget::renderCursor(QPainter &painter, const QRect &cellRect) {
    QColor cursorColor = m_theme.cursorColor.isValid() ? m_theme.cursorColor
                                                        : QColor(0, 255, 0);
...
void ThemePreviewWidget::renderCRTEffect(QPainter &painter, const QRect &area) {
    ...
    if (m_theme.crtGlowRadius > 0.01) {
        ...
        QColor glowColor = m_theme.colorGreen.isValid() ? m_theme.colorGreen
                                                         : QColor(0, 255, 0);
