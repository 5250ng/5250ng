            ...
            for (int i = 0; i < payload.size();) {
                ...
                if (ch == ESC) {
                    ...
                    if (cc == CC_WRITE_TO_DISPLAY) { // Write To Display
                        ...
                        // Legacy signal: unlock keyboard (CC2 bit 3 = 0x08)
                        if (ctrl2 & 0x08) {
                        ...
                        while (j < payload.size()) {
                            ...
                            if (ob == SOH) {
                                ...
                                j += 2 + sohLen;
                                if (j > payload.size()) j = payload.size(); // clamp to avoid overshoot
                                continue;
                            }
                ...
                if (ch == SOH) {
                    ...
                    i += 2 + sohLen;
                    if (i > payload.size()) i = payload.size(); // clamp to avoid overshoot
                    continue;
                }
