...
// ASCII to EBCDIC conversion table (reverse lookup)
const uint8_t EBCDIC::ASCII_TO_EBCDIC[256] = {
...
QChar EBCDIC::ebcdicToChar(uint8_t ebcdic) {
    ...
    if (ascii < 0x20 && ascii != 0x09 && ascii != 0x0A && ascii != 0x0D) {
        ...
        return QChar(0x2400 + ascii); // Use Unicode control picture symbols
    }
    return QChar(ascii);
}
...
uint8_t EBCDIC::charToEBCDIC(QChar ch) {
    ...
    // For non-ASCII characters, return space
...
QString EBCDIC::ebcdicToString(const QByteArray& ebcdic) {
    ...
    for (uint8_t byte : ebcdic) {
        result.append(ebcdicToChar(byte));
    }
    return result;
}
...
bool EBCDIC::isPrintable(uint8_t ebcdic) {
    uint8_t ascii = EBCDIC_TO_ASCII[ebcdic];
    return (ascii >= 0x20 && ascii < 0x7F) || ascii == 0x09 || ascii == 0x0A || ascii == 0x0D;
}
